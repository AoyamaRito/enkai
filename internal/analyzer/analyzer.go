...
func (a *Analyzer) parseResponse(response string) *AnalysisResult {
	...
	// 簡易的なパース（実際はより高度なパースが必要）
	sections := strings.Split(response, "##")
	for _, section := range sections {
		section = strings.TrimSpace(section)
		if strings.HasPrefix(section, "サマリー") {
			result.Summary = strings.TrimSpace(strings.TrimPrefix(section, "サマリー"))
		}
		// 他のセクションも同様にパース
	}
...
// displayResults は結果を表示
func (a *Analyzer) displayResults(result *AnalysisResult) {
	fmt.Println("📊 分析結果")
